...
declare_id!("BCmcy4gVa2heYQ8kqvWbANKP27bAaWHv78fENtm7EHMA");
...
pub mod whitelist_transfer_hook {
    ...
    pub fn initialize_whitelist(ctx: Context<InitializeWhitelist>, user: Pubkey) -> Result<()> {
        ctx.accounts.initialize_whitelist(user, ctx.bumps)
    }
    ...
    pub fn remove_from_whitelist(ctx: Context<WhitelistOperations>, user: Pubkey) -> Result<()> {
        ctx.accounts.remove_from_whitelist(user, &ctx.bumps)
    }
